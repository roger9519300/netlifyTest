...
var $dp, WdatePicker; (function () {
    var _ = {
        ...
        specialDates: null, specialDays: null, disabledDates: null, disabledDays: null, opposite: false, onpicking: null, onpicked: function () {
            ...
            switch (this.id) {
                case "tbxStartDate":
                    var OnShelfDate = document.getElementById('tbxOnShelfDate');
                    if (OnShelfDate == null) { break; }
                    OnShelfDate.value = this.value;
                    break;
                case "tbxEndDate":
                    var OffShelfDate = document.getElementById('tbxOffShelfDate');
                    if (OffShelfDate == null) { break; }
                    var Temp = new Date(this.value);
                    Temp.setMonth(Temp.getMonth() + 1 + 3);
                    TempMonth = "0" + Temp.getMonth();
                    TempDate = "0" + Temp.getDate();

                    OffShelfDate.value = Temp.getFullYear() + "/" + TempMonth.substring(TempMonth.length - 2, TempMonth.length) + "/" + TempDate.substring(TempDate.length - 2, TempDate.length);
                    break;
            }

        }, onclearing: null, oncleared: null, ychanging: null, ychanged: null, Mchanging: null, Mchanged: null, dchanging: null, dchanged: null, Hchanging: null, Hchanged: null, mchanging: null, mchanged: null, schanging: null, schanged: null, eCont: null, vel: null, errMsg: "", quickSel: [], has: {}
